...
### 2026-06-11（Creative Hub 动态编排补全）

Creative Hub 和 Agent Runtime 现在都能更明确地使用动态编排推进整本创作，不再只在局部页面里“看起来像动态”，而是在执行链、审批续跑和状态展示上形成同一套工作方式。

**动态编排执行链补全**

- `produce_novel` 相关的 AI 规划结果现在可以进入动态阶段执行，而不是在通用运行时里被重新压回固定动作表。
- 审批通过后的续跑会保留原来的执行上下文和编排模式，不再把动态计划 silently 退回成普通静态流程。
- 当动态计划连续重规划失败时，系统会明确记录“已从动态编排回退到静态执行”，避免用户只看到结果变化，却不知道执行方式已经切换。

**Creative Hub 状态可见性提升**

- Creative Hub 的回合摘要现在会直接说明当前是动态编排、静态编排，还是“动态失败后转静态”。
- 摘要里会显示当前计划阶段、当前步骤、剩余阶段或步骤数量，以及最近一次重规划或回退的原因。
- 对于等待审批的创作动作，用户现在能更直观看到系统停在什么位置、为什么停下，以及后续会继续推进哪一段创作计划。

**通用 Agent 运行面板补全**

- 通用 Agent 运行详情页现在也会展示编排模式，而不再只能看到工具调用日志。
- 当运行处于动态阶段、重规划阶段或静态回退阶段时，侧边面板会同步展示这些状态，帮助用户区分“只是又调用了工具”与“系统正在重新编排创作路径”这两类不同情况。
